...
First, clone this repo with the name of the desired provider in place of `libvirt`:

```
git clone https://github.com/pulumi/pulumi-tf-provider-boilerplate pulumi-libvirt
```

Second, replace references to `libvirt` with the name of your provider:
...
- `libvirt:apiKey` (environment: `XYZ_API_KEY`) - the API key for `libvirt`
- `libvirt:region` (environment: `XYZ_REGION`) - the region in which to deploy resources
